...
FOR AN ENGINE WITH NO `label` CLASS. best-2.engine was trained on the dataset
with `label` stripped out: it finds the code and the artifact printed on a
label, and nothing that says where the label itself is. Every part of this app
downstream of the detector is written in terms of a label box -- which code
belongs to which label, what gets cropped and saved, whether a label is whole
in the picture, whether one is missing a part -- so the label is put back
before any of that runs.

It is put back by pairing: one code to one artifact, cheapest first, each pair
then held against the median offset of the frame's own pairs so a code whose
artifact was missed cannot reach across to its neighbour's. The box is the
union of the pair plus a margin. utils/labelbox.py does it, the same code
cam-trt-labelbox.py draws with, and the pairs are handed on as ordinary label
detections -- so the window, the sheet, the faults, the crops and the record
all work exactly as they do on a three-class engine, because none of them can
tell the difference.

A label whose partner was never detected still gets a box, put where the
frame's other pairs say it would be. That is not a fudge covering a miss: it
is exactly the label the missing-part check exists to stop the line for, and
it has to reach that check to be stopped for.

A three-class engine still works. When classes.txt names a label class this
build uses it and pairs nothing, so one script covers both.
