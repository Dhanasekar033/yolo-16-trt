...
TWO CAMERAS, ONE WEB, AND AN ENGINE WITH NO `label` CLASS -- run-2cam.py and
run-new.py in one build. best-2.engine finds the code and the artifact printed
on a label and nothing that says where the label is, so the label is put back
by pairing the two parts (utils/labelbox.py) before anything downstream sees
it. That happens per camera, on each camera's own frame: the pairing is
measured inside one picture -- which artifact sits below which code, how far
apart the pairs of this frame sit -- and two cameras are two pictures.

Everything after that is the two-camera build unchanged. A code read by either
camera ticks its own cell, because matching is by payload.

TWO CAMERAS, ONE WEB. A build of run.py for a web wider than one camera can
resolve: camera A covers some of the ups across it, camera B the rest, and
between them they see every label. There is still one sheet, one rolling
window, one relay and one record -- a row is complete when its codes have been
read, and which camera read them does not come into it.

That works without a single positional merge because this app matches by
payload: a code identifies its own sheet row and column, so a code read by
either camera ticks its own cell. What DOES have to be kept apart is
everything the machine measures from a picture -- how fast the web is running,
which labels are in frame, which crops are waiting for a clean look, which
labels are missing a part. Two cameras see two different pictures, and mixing
...
from utils.labelbox import find_labels
from utils.scanlog import DATAMATRIX, FIELDS as SCAN_FIELDS, QR, ScanLog
...
class Cam:
    """One camera: its stream, its thread, and the machine's state for it.

    The state is the part worth explaining. `motion`, `last_labels`, `marks`,
    `parts`, `handled`, `pending_crops` and `tracks` are all measured from a
    picture, and each camera has its own picture -- so each carries its own
    set, and the pass swaps the right one in before it works on that camera's
    frame. Sharing them would have the web's speed computed from labels that
    are not on the same part of it, and crops waiting on a frame they were
    never in.

    The thread does nothing but keep the newest frame. It never touches the
    machine, and the machine never blocks on a camera: a stream that stalls
    stops offering frames, and the other one carries on.
    """

    def __init__(self, tag, cap, rotate=0, name="", index=None, spec=None):
        ...
        self.cap = cap
        self.rotate = rotate
        # WHAT THIS CAMERA IS, rather than what "the camera" is. A 2MP and a
        # 5MP over one web deliver different pictures, so size, rate and the
        # modes the driver offers are properties of a camera here and not of
        # the run. `spec` is what was asked for; w/h is what actually turned
        # up, measured off the first frame -- which is the only number worth
        # laying the screen out from, because a driver that quietly gave us
        # something else would otherwise put every box in the wrong place.
        self.spec = spec or {}
        self.w = self.spec.get("width", 0)
        self.h = self.spec.get("height", 0)
        if rotate in (90, 270):
            self.w, self.h = self.h, self.w
        self.rates = self.spec.get("rates") or {}
        # Where this camera's pane sits on the composed picture, and what
        # its own pixels are multiplied by to get there. Filled in once the
        # sizes are known; one scale per camera because two cameras of
        # different sizes cannot share one.
        self.scale = 1.0
        self.pane_x = 0
        self.pane_w = 0
        self.frame = None              # the frame this pass is working on
        self.shown = None              # ... with the drawing on it
        self.newest = None             # what the thread has ready
        self.lock = threading.Lock()
        self.stop = threading.Event()
        self.thread = None
        self.misses = 0
        self.frames = 0
        self.state = {"last_labels": [], "motion": (0.0, 0.0), "marks": [],
                      "parts": [], "handled": [], "pending_crops": [],
                      "tracks": []}

    def start(self):
        self.thread = threading.Thread(target=self._grab, daemon=True,
                                       name=f"grab-{self.tag}")
        self.thread.start()
    ...
    def _grab(self):
        while not self.stop.is_set():
            ...
            with self.lock:
                # Only the newest is kept: a pass that took longer than a
                # frame should carry on from where the web is now, not work
                # through a backlog of where it has been.
                self.newest = frame

    def measure(self, timeout=6.0):
        """Wait for one frame and take the picture size off it.

        Asked of the stream rather than worked out from the arguments. A
        camera can negotiate a mode other than the one requested, a file is
        whatever it was recorded at, and either way the console has to lay
        out the size that is actually going to arrive. The frame is left
        where it is -- this only looks.
        """
    ...
    def take(self):
        """The newest frame, once. None when nothing new has arrived."""
        with self.lock:
            frame, self.newest = self.newest, None
        if frame is not None:
            self.frames += 1
            self.frame = frame
        return frame
...
def main():
    ...
    # A SECOND CAMERA THAT IS NOT THE SAME CAMERA. Left out, each of these
    # follows camera A, which is the matched pair this rig usually runs. Set
    # them and camera B is opened at its own size and rate -- a 2MP beside a
    # 5MP, say -- and the console lays out two panes of different shapes.
    ap.add_argument("--width-b", type=int, default=None,
                     help="camera B's width, when it is not the same camera "
                          "as A. Default: --width.")
    ap.add_argument("--height-b", type=int, default=None,
                     help="camera B's height. Default: --height.")
    ap.add_argument("--fps-b", type=int, default=None,
                     help="camera B's frame rate. Default: --fps. Each camera "
                          "is held to what its own driver offers at its own "
                          "size, so a 90/s 2MP and a 60/s 5MP each run at "
                          "their best rather than both at the slower one.")
    ap.add_argument("--rotate-b", type=int, default=None,
                     choices=[0, 90, 180, 270],
                     help="camera B's rotation. Default: --rotate.")
    ap.add_argument("--index-b", type=int, default=None,
    ...
    ap.add_argument("--source-b", default=None,
                     help="a recording to stand in for camera B, the way "
                          ...
                          "build is tested without two cameras on the bench.")
    ...
    ap.add_argument("--box-gutter", action="store_true",
                     help="build the label box out to the edges of the card "
                          "instead: widen into the gap between the labels by "
                          "half of whatever room is measured there. Half is "
                          "the most that can never take in the label next "
                          "door.")
    ap.add_argument("--box-axis", choices=["auto", "x", "y"], default="auto",
                     help="which way a label's two parts are stacked. "
                          "Measured off the frame by default, so a camera "
                          "turned on its side needs nothing said.")
    ...
    def set_format(yuyv):
        ...
        # THE OLD STREAMS GO FIRST, and they have to. V4L2 makes streaming
        # exclusive: while this app holds a device, nothing -- including this
        # app -- can open it again, so "start the new one and keep the old if
        # it fails" cannot work. It fails every time, with 'device busy', and
        # the message blames the format rather than the order.
        #
        # Both cameras change together or neither does. Two cameras over one
        # web running different formats would put half the crops at 60 frames
        # a second and half at 5, and every measurement the machine makes
        # from a picture would mean something different on each half.
        was_fmt = args.format
        for cam in cams:
            cam.stop.set()
        ...
        opened, failed = [], None
        for cam in cams:
            # Each at its own size and its own best rate for this format.
            # One format across the pair, because that is a choice about the
            # job -- compressed or the sensor's own pixels -- but the size
            # and the rate stay the camera's own.
            fresh, rot = open_camera(cam.index, fmt, cam.spec, tries=2)
            if fresh is None:
                failed = cam.tag
                break
            opened.append((cam, fresh, rot))
        ...
        if failed is not None:
            ...
            back = []
            for cam in cams:
                again, rot = open_camera(cam.index, was_fmt, cam.spec,
                                         tries=3)
                if again is None:
                    print(f"[cam{cam.tag}] {fmt} would not open, and "
                          f"{was_fmt} did not come back — the camera has "
                          f"stopped answering")
                    _note[0] = "A camera stopped answering — restart the app"
                    return
                back.append((cam, again, rot))
            for cam, again, rot in back:
                cam.cap, cam.rotate = again, rot
                cam.stop.clear()
                cam.start()
            spec = next((c.spec for c in cams if c.tag == failed),
                        cam_specs["A"])
        ...
        for cam, fresh, rot in opened:
            cam.cap, cam.rotate = fresh, rot
            cam.stop.clear()
            cam.start()
        args.format = fmt
        prefs.remember_format(fmt)
        rates = [best_fps(c.spec["rates"], fmt, c.spec["fps"]) for c in cams]
        if len(set(rates)) > 1:
            # Different cameras, different best rates. Saying "60/s" when
            # half the web is at 90 is the kind of readout that has somebody
            # looking for a fault in the wrong place.
            detail = ", ".join(f"cam{c.tag} {r}/s"
                               for c, r in zip(cams, rates))
            print(f"\n[camera] {fmt} — {detail}")
            _note[0] = f"{fmt}: {detail}"
        else:
            rate = rates[0]
            print(f"\n[camera] "
                  + ("both cameras on" if len(cams) > 1 else "camera on")
                  + f" {fmt} at {rate}/s")
    ...
    # Which devices the two cameras are. Grouped by the USB device each node
    # hangs off, because one camera presents several /dev/videoN -- this rig's
    # presents video0 for pictures and video1 for metadata -- and counting
    # nodes would open a metadata node as camera B and find no frames on it.
    def capture_devices():
        by_device, order = {}, []
        for entry in sorted(os.listdir("/dev")):
            ...
            try:
                device = os.path.realpath(
                    f"/sys/class/video4linux/{entry}/device")
            except OSError:
                device = entry
        ...
        return [min(by_device[d]) for d in order]

    devices = [] if (args.source and args.source_b) else capture_devices()
    index_a = args.index if args.index is not None else \
        (devices[0] if devices else 0)
    index_b = args.index_b if args.index_b is not None else \
        next((i for i in devices if i != index_a), None)

    # What each format is worth at this size, asked of the driver rather than
    # assumed. Probed on camera A: a matched pair is the only sane way to run
    # two cameras over one web, and a mismatched one has bigger problems than
    # its frame rate.
    def spec_for(tag):
        """What camera `tag` is being asked for: size, rate, and the modes
        its own driver offers at that size.

        Asked of each device separately. What a format is worth is a
        property of the camera and the size it is running at -- this rig's
        5MP does MJPG 60/s and YUYV 35/s at full frame, the 2MP beside it
        does 90/s and 5/s -- so probing one and applying the answer to both
        would run one of them at a rate it does not have, which does not
        fail politely: the pipeline simply never starts.
        """
        if tag == "A":
            width, height = args.width, args.height
            fps, rotate, index = args.fps, args.rotate, index_a
            source = args.source
        else:
            width = args.width_b if args.width_b is not None else args.width
            height = args.height_b if args.height_b is not None else args.height
            fps = args.fps_b if args.fps_b is not None else args.fps
            rotate = args.rotate_b if args.rotate_b is not None else args.rotate
            index, source = index_b, args.source_b
        device = (CAM["device"] if tag == "A" and CAM["device"]
                  else (f"/dev/video{index}" if index is not None else None))
        rates = {} if (source or not device) else list_rates(device, width,
                                                             height)
        return {"width": width, "height": height, "fps": fps,
                "rotate": rotate, "rates": rates, "device": device}

    cam_specs = {"A": spec_for("A"), "B": spec_for("B")}
    def best_fps(rates, fmt, wanted):
        """The best rate this format offers at this size, up to `wanted`.

        A format that cannot do what was asked is run at the fastest it has
        rather than refused: YUYV at 1920x1200 on the 2MP is 5/s, and 5
        uncompressed frames a second is exactly what somebody choosing YUYV
        is asking for.
        """
        have = (rates or {}).get(fmt.upper()) or []
        if not have:
            return int(wanted)
        fits = [r for r in have if r <= wanted + 0.01]
        return int(max(fits) if fits else min(have))

    def format_fps(fmt, spec=None):
        spec = spec if spec is not None else cam_specs["A"]
        return best_fps(spec["rates"], fmt, spec["fps"])

    def _pair_rates():
        """What each format is worth across the whole run: the slowest
        camera's best. Cameras that are recordings have nothing to say."""
        have = [c.spec["rates"] for c in cams if c.spec.get("rates")]
        if not have:
            return {}
        formats = set().union(*(set(r) for r in have))
        out = {}
        for fmt in formats:
            best = [max(r[fmt]) for r in have if r.get(fmt)]
            if len(best) == len(have):      # every camera offers it
                out[fmt] = min(best)
        return out

    def open_camera(index, fmt, spec=None, tries=1):
        ...
        fps = best_fps(spec["rates"], fmt, spec["fps"])
        for attempt in range(max(1, tries)):
            if attempt:
                time.sleep(0.4)
            pipeline = gstreamer_pipeline(index, width, height,
                                          fps, fmt, rotate=rotate)
            print(f"[camera] pipeline: {pipeline}")
            fresh = cv2.VideoCapture(pipeline, cv2.CAP_GSTREAMER)
            rot = 0
            if not fresh.isOpened() and rotate:
                ...
                print("[camera] videoflip pipeline would not open — rotating "
                      "in the loop")
                pipeline = gstreamer_pipeline(index, width,
                                              height, fps, fmt)
                fresh = cv2.VideoCapture(pipeline, cv2.CAP_GSTREAMER)
                rot = rotate
            if fresh.isOpened():
    ...
    def open_stream(tag, source, index):
        """One camera, or the recording standing in for it."""
        spec = cam_specs[tag]
        if source:
            cap = FrameSource(source, fps=args.source_fps,
                              speed=args.source_speed, loop=args.source_loop)
            print(f"[cam{tag}] {cap.describe()}")
            return Cam(tag, cap, spec["rotate"], os.path.basename(source),
                       spec=spec)
        if index is None:
            raise SystemExit(
        ...
        cap, rot = open_camera(index, args.format, spec)
        if cap is None:
            raise RuntimeError(f"[cam{tag}] could not open /dev/video{index}"
                               f" — is another process holding it?")
        rates = spec["rates"]
        print(f"[cam{tag}] {spec['width']}x{spec['height']} {args.format} at "
              f"{best_fps(rates, args.format, spec['fps'])}/s"
              + ("  (" + ", ".join(
                  f"{k} up to {max(v):g}/s"
                  for k, v in sorted(rates.items())) + ")"
                 if rates else ""))
        return Cam(tag, cap, rot, f"/dev/video{index}", index=index, spec=spec)
    ...
    for cam in cams:
        cam.start()
    # WHAT ACTUALLY ARRIVES, waited for rather than assumed. The console is
    # laid out from these, and a camera that negotiated a mode other than the
    # one asked for would otherwise have every box drawn in the wrong place.
    for cam in cams:
        if not cam.measure():
            print(f"[cam{cam.tag}] no frame yet — laying the screen out at "
                  f"{cam.w}x{cam.h}, which is what was asked for")
    if len(cams) > 1:
        print(f"[run] {len(cams)} streams: "
              + ", ".join(f"cam{c.tag} {c.name} {c.w}x{c.h}" for c in cams))
        if len({(c.w, c.h) for c in cams}) > 1:
            # Worth saying plainly. Two cameras of different sizes over one
            # web is a supported arrangement, not a misconfiguration -- but
            # it does mean the crops off one half of the web carry more
            # detail than the other, and that is a thing to know before
            # somebody wonders why.
            print(f"[run] the streams are different sizes — each gets its own "
                  f"pane, and each is held to what its own driver offers. "
                  f"Crops from the larger one will carry more detail.")
    ...
    if remembered:
        applied = {}
        for controls in cameras:
            applied.update(controls.apply(remembered) or {})
        if applied:
    ...
    def _set_camera(arg):
        ...
        try:
            ...
            return
        # Set on both cameras: they are looking at one web in one light, and
        # half a web exposed differently from the other half is a difference
        # that shows up in the crops and nowhere on the screen.
        took = [controls.set(name, value) for controls in cameras]
        if not any(took):
    ...
    # THE COMPOSED PICTURE, laid out from what the cameras actually deliver.
    #
    # Two cameras of different sizes cannot share a scale, so each pane is
    # brought to one common height and keeps its own width from its own
    # aspect. The height is the SMALLEST of them: bringing the small camera
    # up to the big one would invent pixels it never had, and the whole point
    # of the larger sensor is the detail it really has.
    #
    # disp_w/disp_h stay what they always were -- the size of the composed
    # picture before the screen cap -- so everything that draws chrome on it
    # (the fault banner, the console panel, the recorder) needs no changes.
    _pane_h = min(c.h for c in cams if c.h) or args.height
    for _c in cams:
        _c.native_w = max(1, int(round(_c.w * _pane_h / _c.h))) if _c.h else _c.w
    disp_w = sum(_c.native_w for _c in cams)
    disp_h = _pane_h
    ...
    if paired:
        ...
        # The synthesised class sits past the end of the real ones, so it can
        # never collide with something the engine actually emits.
        label_cls = len(class_names)
        class_names = list(class_names) + [args.label_class]
        print(f"[model] no '{args.label_class}' class in this engine — label "
              f"boxes are built from "
              f"{class_names[qr_cls]} + {class_names[logo_cls]} pairs")
    else:
    ...
    def _start_record():
        ...
        if not args.no_scan_log:
            # One extra column: which camera read the code. With two of them
            # over one web, a record that does not say is a record that
            # cannot be used to work out which half of the web a bad read
            # came from.
            scanlog = ScanLog(root=args.label_dir, name=run_name,
                              fields=SCAN_FIELDS + ("camera",))
        results = None
    ...
    def capture_frame():
        ...
        """
        if all(cam.frame is None for cam in cams):
            print("[capture] no frame to save yet")
            _note[0] = "No picture to capture yet"
            return
        # One stamp across both files, so the pair is recognisable afterwards
        # as one moment: two cameras over one web are photographing the same
        # instant of it, and a record where that has to be worked out from
        # two clocks is a record nobody trusts.
        stamp = timestamp()
        ...
        for cam in cams:
            ...
            if frame is None:
                continue
            name = f"capture_{stamp}_cam{cam.tag}.{args.label_format}"
            ...
            if not ok:
                ...
                return
            written.append(name)
            shapes.append(f"{frame.shape[1]}x{frame.shape[0]}")
        print(f"[capture] {', '.join(shapes)} saved to "
              f"{args.capture_dir}/ ({', '.join(written)})")
        _note[0] = f"Saved {len(written)} picture(s)"
        return
    ...
    scale = min(DISP["max_width"] / disp_w,
                DISP["max_height"] / disp_h, 1.0)
    pane_h = max(1, int(disp_h * scale))
    # Where each camera's pane starts on the composed picture, and what its
    # own pixels are multiplied by to land there. One scale per camera: a
    # 2MP and a 5MP brought to the same height are multiplied by different
    # numbers, and a single shared `scale` is exactly the assumption that
    # put the boxes in the wrong pane.
    _x = 0
    for _c in cams:
        _c.pane_w = max(1, int(round(_c.native_w * scale)))
        _c.scale = _c.pane_w / float(_c.w) if _c.w else scale
        _c.pane_x = _x
        _x += _c.pane_w
    _comp_w = _x                         # what compose() actually returns

    def _stack(frames, height, widths):
        """Panes side by side at one height, each keeping its own shape.

        Every pane is resized to exactly its own slot, so the concatenation
        always fits -- no cropping to the shortest, which on two cameras of
        different sizes would quietly cut the bottom off the taller one.
        """
        panes = []
        for frame, width in zip(frames, widths):
            if frame is None:
                panes.append(np.zeros((height, width, 3), np.uint8))
                continue
            if frame.shape[0] != height or frame.shape[1] != width:
                frame = cv2.resize(frame, (width, height),
                                   interpolation=cv2.INTER_NEAREST)
            panes.append(frame)
        return cv2.hconcat(panes) if len(panes) > 1 else panes[0]

    def lay_out(frames):
        """At the size the screen will show."""
        return _stack(frames, pane_h, [c.pane_w for c in cams])

    def lay_out_native(frames):
        """At the composed picture's own size -- disp_w x disp_h, which is
        what the chrome, the recorder and the OpenCV console are drawn
        against."""
        return _stack(frames, disp_h, [c.native_w for c in cams])

    def compose():
        """Every camera side by side, at the size they will be shown.

        A camera that had no new frame this pass keeps showing its last one:
        they run at their own rates, and blanking a pane every time its
        camera was a millisecond late would make the screen flicker between
        one picture and two.
        """
        return lay_out([cam.frame for cam in cams])
    ...
    cam_now = [None]         # whose frame the machine is looking at

    class _BoxOpts:
        """What utils/labelbox.py needs to know to shape a label box.

        A object rather than the parser's own namespace because the two
        vocabularies collide: --label-pad is the margin round a saved CROP and
        has been since before any of this, while the pad here is what makes
        the label box itself. Keeping them apart is the difference between
        widening a picture and widening the thing the machine reasons about.
        """
        pad = args.box_pad
        pad_px = args.box_pad_px
        gutter = args.box_gutter
        gutter_share = 0.5
        gutter_cap = 0.6
        axis = args.box_axis
        # Never dropped. A label whose partner was missed is exactly what the
        # missing-part check stops the line for, and a label dropped here
        # would be a label that check never sees -- the code on it would go
        # unread and the row would come up short, which is the same stop for
        # the wrong reason.
        no_singles = False
    ...
    def _with_label_boxes(dets, shape):
        ...
        Appended rather than substituted: the code and artifact boxes are
        still what gets cropped and decoded, and still what the missing-part
        ...
        if not labels:
            return dets
        made = np.array([[box[0], box[1], box[2], box[3],
                          1.0 if measured else 0.5, label_cls]
                         for box, measured in labels], dtype=np.float32)
        return np.concatenate([dets, made]) if len(dets) else made

    def _swap_in(cam):
        """Make the machine's per-picture state this camera's.

        Every one of these is measured from a frame -- how far the web moved,
        which labels were in it, which crops are still waiting for a clean
        look at one. Two cameras see two different parts of the web, so each
        keeps its own set and the pass swaps the right one in around its own
        frame. Everything else the machine knows -- the window, the sheet, the
        fault, the record -- is the web's, not a camera's, and is shared.
        """
        cam_now[0] = cam
        s = cam.state
        last_labels[0] = s["last_labels"]
        motion[0] = s["motion"]
        marks[0] = s["marks"]
        parts[0] = s["parts"]
        handled[0] = s["handled"]
        pending_crops[:] = s["pending_crops"]
        tracks[:] = s["tracks"]
        last_frame[0] = cam.frame

    def _swap_out(cam):
        s = cam.state
        s["last_labels"] = last_labels[0]
        s["motion"] = motion[0]
        s["marks"] = marks[0]
        s["parts"] = parts[0]
        s["handled"] = handled[0]
        s["pending_crops"] = list(pending_crops)
    ...
    def process_frame():
        ...
        the grabbing is what runs in parallel, which is the part that would
        otherwise have one camera waiting on the other.
        """
        grabbed = []
        for cam in cams:
            frame = cam.take()
            ...
            dets = postprocess(raw, ratio, pad, frame.shape, args.conf_thres,
                               conf_per_class)
            # The label put back, per camera: the pairing is measured inside
            # one picture, so it has to happen on each camera's own frame
            # before anything downstream sees a label at all.
            dets = _with_label_boxes(dets, frame.shape)
            grabbed.append((cam, frame, dets))
        ...
        #
        # Cleared once for the pass rather than once per camera: it means
        # "the offending code is in front of a camera now", and either camera
        # seeing it is enough to keep the fault alive.
        fault["in_frame"] = False
        for cam, frame, dets in grabbed:
            _swap_in(cam)
            if scanning():
        ...
        if scanning() and starting_at[0] is not None \
                and time.time() - starting_at[0] >= args.start_delay:
            _finish_start()
        # Once for the pass: "nothing has read for N seconds" is a fact about
        # the web, not about a camera, and asking it twice would halve the
    ...
    def _snapshot(grabbed=()):
        ...
        for cam in cams:
            # This camera's own offset and its own scale. Two cameras of
            # different sizes are multiplied by different numbers to reach
            # the same pane height, so neither can be taken from the run.
            dx, k = cam.pane_x, cam.scale

            def at(box, dx=dx, k=k):
                x1, y1, x2, y2 = (v * k for v in box)
                return (x1 + dx, y1, x2 + dx, y2)

            for box, who, text in cam.state["marks"]:
                x1, y1, x2, y2 = at(box)
                bad = _bad_label(box)
                colour = (DECODER_COLOUR["fail"] if bad
                          else DECODER_COLOUR.get(who, DECODER_COLOUR["fail"]))
            ...
            part_boxes += [at(box) + (_rgb(PART_COLOUR[kind]),)
                           for box, kind in cam.state["parts"]]

            if fault["kind"] is not None:
                for box, _who, text in cam.state["marks"]:
                    tag = _fault_tag(box, text)
                    if tag is None:
                        continue
                    caption, colour = tag
        ...
        return {
            ...
            "format": {"live": not bool(args.source),
                       ...
                       # Across every camera in the run, not just the first.
                       # With a 2MP beside a 5MP the two have different
                       # rates, and the readout takes the slower -- a hint
                       # that promised 90/s while half the web ran at 60
                       # would send somebody looking for a fault that is not
                       # there.
                       "rates": _pair_rates(),
                       "fps": min(best_fps(c.spec["rates"], args.format,
                                           c.spec["fps"]) for c in cams)},
            "debug": show_debug[0],
            ...
            # Both cameras' detections: the readout answers "is the model
            # seeing anything", and with two cameras that is a question
            # about the pair.
            "fps": fps_state[1],
            "dets": sum(len(d) for _cam, _frame, d in grabbed),
            "status": _status_line(),
    ...
    def _loop_opencv():
        ...
        while True:
            ...
            if got is None:
                continue
            dets = [d for _cam, _f, ds in got for d in ds]

            # Drawn per camera, with that camera's verdicts swapped in: the
            # boxes are in the coordinates of the frame they were measured
            # in, so each has to go onto its own picture before the two are
            # put side by side.
            for cam, cam_frame, cam_dets in got:
                _swap_in(cam)
                shown = draw_detections(cam_frame.copy(), cam_dets, class_names)
                if scanning():
                    draw_decoders(shown, legend=show_debug[0])
                cam.shown = shown
                _swap_out(cam)
            panes = [(cam.shown if cam.shown is not None else cam.frame)
                     for cam in cams]
            if any(pane is None for pane in panes):
                continue
            frame = lay_out_native(panes)
            draw_fault(frame)
            if show_debug[0]:
            ...
            if args.no_display:
                _headless_line(frame, dets)   # both panes, both cameras' dets
            else:
                if show_debug[0]:
                    cv2.putText(frame,
