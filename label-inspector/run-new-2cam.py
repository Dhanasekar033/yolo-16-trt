...
def main():
    ...
    cam_rates = ({} if args.source else
                 list_rates(CAM["device"] or f"/dev/video{index_a}",
    ...
    def open_camera(index, fmt, tries=1):
        ...
        for attempt in range(max(1, tries)):
            ...
            if not fresh.isOpened() and args.rotate:
                ...
                print("[camera] videoflip pipeline would not open — rotating "
                      "in the loop")
                pipeline = gstreamer_pipeline(index, args.width,
                                              args.height, fps, fmt)
                fresh = cv2.VideoCapture(pipeline, cv2.CAP_GSTREAMER)
                rot = args.rotate
            if fresh.isOpened():
    ...
    def open_stream(tag, source, index):
        ...
        if source:
            ...
            return Cam(tag, cap, args.rotate, os.path.basename(source))
        if index is None:
            raise SystemExit(
        ...
        print(f"[cam{tag}] {args.format} at {format_fps(args.format)}/s"
              + ("  (" + ", ".join(
                  f"{k} up to {max(v):g}/s"
    ...
    def compose():
        """Both cameras side by side, at the size they will be shown.

        A camera that had no new frame this pass keeps showing its last one:
        the two run at their own rates, and blanking a pane every time its
        ...
        for cam in cams:
            frame = cam.frame
            if frame is None:
                panes.append(np.zeros((int(disp_h * scale), pane_w, 3),
                                      np.uint8))
                continue
            panes.append(cv2.resize(frame, None, fx=scale, fy=scale,
                                    interpolation=cv2.INTER_NEAREST)
                         if scale < 1.0 else frame)
        height = min(p.shape[0] for p in panes)
        panes = [p[:height] for p in panes]
    ...
    def _loop_opencv():
        ...
        while True:
            ...
            frame = cv2.hconcat([pane[:height] for pane in panes])
            draw_fault(frame)
            if show_debug[0]:
